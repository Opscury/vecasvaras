...
export class TitleScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // not something to continue: after the ending, Begin starts afresh.
    const run = state.get();
    const hasRun = run.introSeen && !run.outroSeen;
    const startLabel = hasRun ? ui.resume : ui.begin;
  ...
  private begin(): void {
    ...
    // After the ending, Begin means a new run, not a walk back into the old one.
